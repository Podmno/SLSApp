...
SGModalSource::SGModalSource(SGModalSourceType type)
{
    // 默认设置类型 sourceV
...
void SGModalSource::setModalSourceType(SGModalSourceType type)
{
    modalValue.empty();
    modalValueDescription.empty();
    
    if(type == SGModalSourceType::SourceTypeV) {
        modalValue.append("5.0");
        modalValueDescription.append(SGModalValueDescription(SGModalValueType::ModalValueTypeDouble, "AC 电压源的电压 / 伏特"));
        itemType = SGItemType::ItemTypeSourceV;
        itemIdentifier = "SG.SourceV";
    }
    if(type == SGModalSourceType::SourceTypeA) {
        modalValue.append("5.0");
        modalValueDescription.append(SGModalValueDescription(SGModalValueType::ModalValueTypeDouble, "电流源的电流 / 安培"));
...
void SGModalSource::drawItem(QWidget *paintWidget, SGCanvasViewInfo viewInfo)
{
